 ...
 */
public class MovieUpcomingPresenter extends RxPresenter<DoubanMovieUpcomingContract.View> implements DoubanMovieUpcomingContract.Presenter {
    ...
    @Inject
    public MovieUpcomingPresenter(RetrofitHelper retrofit) {
        mRetrofit = retrofit;
    }
    ...
    public void getData() {
        mView.showProgress();
        addSubscribe(mRetrofit.mDoubanApiService.getUpcoming(0, PER_LOAD)
    ...
    public void getMoreData(int start) {
        ...
        isLoadMoreData = true;
        mView.showProgress();
        addSubscribe(mRetrofit.mDoubanApiService.getUpcoming(start, PER_LOAD)
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .subscribe(new Action1<MovieListBean>() {
                    @Override
