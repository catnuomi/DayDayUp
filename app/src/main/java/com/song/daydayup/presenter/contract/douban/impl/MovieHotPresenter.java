 ...
 */
public class MovieHotPresenter extends RxPresenter<DoubanMovieHotContract.View> implements DoubanMovieHotContract.Presenter {
    ...
    @Inject
    public MovieHotPresenter(RetrofitHelper retrofit) {
        mRetrofit = retrofit;
    }
    ...
    public void getData() {
        mView.showProgress();
        addSubscribe(mRetrofit.mDoubanApiService.getHot(0, PER_LOAD)
    ...
    public void getMoreData(int start) {
        ...
        isLoadMoreData = true;
        addSubscribe(mRetrofit.mDoubanApiService.getHot(start, PER_LOAD)
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .subscribe(new Action1<MovieListBean>() {
                    @Override
